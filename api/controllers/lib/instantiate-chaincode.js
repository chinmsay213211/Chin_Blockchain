...
hfc.addConfigFile(path.join(__dirname, config.network));
...
var instantiateChaincode = function(channelName, chaincodeName, chaincodeVersion, functionName, args, username, org) {
	...
	return helper.getOrgAdmin(org).then((user) => {
		...
		if (all_good) {
			...
			var data = fs.readFileSync(path.join(__dirname, ORGS[org]['peer1'][
				'tls_cacerts'
			]));
			...
			var txPromise = new Promise((resolve, reject) => {
				...
				eh.registerTxEvent(deployId, (tx, code) => {
					logger.info(
                        chaincodeName + ' instantiate transaction has been committed on peer ' +
		...
		if (response.status === 'SUCCESS') {
			logger.info(chaincodeName + ' successfully sent transaction to the orderer.');
			return chaincodeName + ' Instantiateion is SUCCESS';
		} else {
			logger.error(chaincodeName + ' failed to order the transaction. Error code: ' + response.status);
			return chaincodeName + ' failed to order the transaction. Error code: ' + response.status;
		}
	}, (err) => {
		logger.error(chaincodeName + ' failed to send instantiate due to error: ' + err.stack ? err
			.stack : err);
		return chaincodeName + ' failed to send instantiate due to error: ' + err.stack ? err.stack :
			err;
	});
};
